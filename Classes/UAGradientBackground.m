...
- (id)initWithFrame:(CGRect)frame style:(UAGradientBackgroundStyle)aStyle color:(CGFloat *)components lineMode:(UAGradientLineMode)lineModes {
	if ((self = [super initWithFrame:frame])) {
		...
		lineMode = lineModes;
		colorComponents = (float*)NSZoneMalloc(NSDefaultMallocZone(), 8*sizeof(CGFloat));
		for (int i = 0; i < 8; i++) {
...
- (id)initWithFrame:(CGRect)frame color:(CGFloat *)components {
	return [self initWithFrame:frame style:UAGradientBackgroundStyleLinear color:components lineMode:UAGradientLineModeNone];
}
