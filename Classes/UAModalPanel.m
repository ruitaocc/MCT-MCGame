...
#define DEFAULT_SHOW_CLOSE			YES

@implementation UAModalPanel

@synthesize roundedRect, closeButton, delegate, contentView, contentContainer;
@synthesize innerMargin, x_outerMargin,y_outerMargin, cornerRadius, borderWidth, borderColor, contentColor, shouldBounce,isShowColseBtn;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self != nil) {
		...
		x_outerMargin = DEFAULT_MARGIN;
        y_outerMargin = DEFAULT_MARGIN;
		...
		isShowColseBtn = DEFAULT_SHOW_CLOSE;
...
- (CGRect)roundedRectFrame {
	...
	return CGRectMake(self.x_outerMargin + self.frame.origin.x,
					  self.y_outerMargin + self.frame.origin.y,
					  self.frame.size.width - 2*self.x_outerMargin,
					  self.frame.size.height - 2*self.y_outerMargin);
}

- (CGRect)closeButtonFrame {
	CGRect f = [self roundedRectFrame];
    return CGRectMake(f.origin.x+f.size.width-floor(closeButton.frame.size.width*0.5), f.origin.y-floor(closeButton.frame.size.height*0.5), closeButton.frame.size.width,
